...
# Set base directory to Downloads
...
def load_components():
    try:
        ...
        # Get season options (exact names from data)
...
def main():
    ...
    if submitted:
        try:
            ...
            # Get average yield for the crop (for cluster prediction)
            avg_logyield = components['avg_yields'][
                components['avg_yields']['Crop'] == crop_encoded
            ]['LogYield'].mean()
            
            # Scale the yield (as done in training)
            scaled_logyield = components['logyield_scaler'].transform(
                [[avg_logyield]]
            )[0][0]
            
            # Create season dummy (exact match with training)
            ...
            # Prepare production prediction input
            input_data = {
                'Area': area,
